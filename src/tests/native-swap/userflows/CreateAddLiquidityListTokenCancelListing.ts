...
await opnet('Native Swap - User flows', async (vm: OPNetUnit) => {
    ...
    await vm.it('should run CreateAddLiquidityListTokenCancelListing correctly', async () => {
        const player = new ScenarioPlayer();
        await player.runScenarioFile('./scenarios/CreateAddLiquidityListTokenCancelListing.json');
    });
});
