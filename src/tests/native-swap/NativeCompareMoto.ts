...
await opnet('Compare NativeSwap vs Normal OP20 Swap', async (vm: OPNetUnit) => {
    ...
    await vm.it('compare with Motoswap', async () => {
        ...
        await addMotoswapLiquidity(point25InitialLiquidity / floorPrice, point25InitialLiquidity);

        await runMotoswapScenario(1);
    ...
    async function addMotoswapLiquidity(tokenAAmount: bigint, tokenBAmount: bigint) {
        ...
        recordCandle(
            Blockchain.blockNumber,
            tokenBAmount / tokenAAmount, // raw bigint
            data,
        );
     ...
     * AND also record a candle point in `data`.
     */
    async function runNativeSwapScenario2(count: number) {
        /*for (let y = 0; y < 5; y++) {
     ...
     * AND also record a candle point in `data`.
     */
    async function runMotoswapScenario2(count: number) {
        const wbtcIn = satoshisIn;
