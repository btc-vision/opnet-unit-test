...
await opnet('NativeSwap: Priority and Normal Queue listLiquidity', async (vm: OPNetUnit) => {
  ...
  const stakingAddress: Address = Blockchain.generateRandomAddress();
  ...
  vm.beforeEach(async () => {
    ...
    // Initiate and register dummy staking contract
    staking = new DummyStakingPool(userAddress, stakingAddress)
    Blockchain.register(staking);
    await staking.init()

    const backup = Blockchain.msgSender;
    Blockchain.msgSender = userAddress;
    await nativeSwap.setStakingContractAddress(stakingAddress)
  ...
  await vm.it(
    ...
    async () => {
      ...
      // resetStates messes with staking contract address causing this test to fail, so we re-set it
      const backup = Blockchain.msgSender;
      Blockchain.msgSender = userAddress;
      await nativeSwap.setStakingContractAddress(stakingAddress)
      Blockchain.msgSender = backup;
