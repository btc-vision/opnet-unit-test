...
export function decodeListLiquidityEventsHelper(events: NetEvent[]): ListLiquidityEventsHelper {
    const result: ListLiquidityEventsHelper = new ListLiquidityEventsHelper();
...
export function assertListLiquidityEventsHelper(
    nativeSwapContractAddress: Address,
    stakingContractAddress: Address,
    provider: ProviderHelper,
    amountIn: bigint,
    tax: bigint,
    events: ListLiquidityEventsHelper,
): void {
    ...
    Assert.expect(events.transferredEvents[0].from.toString()).toEqual(provider.address.toString());
