...
await opnet('Motoswap Pool', async (vm) => {
    const token0Address = Blockchain.generateRandomSegwitAddress();
    const token1Address = Blockchain.generateRandomSegwitAddress();
    ...
    /** Init OP_20 */
    const token0 = new OP_20('MyToken', Blockchain.txOrigin, token0Address, 18);
    Blockchain.register(token0);
    const token1 = new OP_20('MyToken', Blockchain.txOrigin, token1Address, 18);
    ...
    await vm.it('should return the correct token0 and token1', async () => {
        const _token0 = await pool.getToken0();
        const _token1 = await pool.getToken1();
    ...
    await vm.it('should mint liquidity', async () => {
        const amountTokenA = Blockchain.expandToDecimal(1, token0.decimals);
        const amountTokenB = Blockchain.expandToDecimal(4, token1.decimals);
        await token0.transfer(receiver, pool.address, amountTokenA);
        await token1.transfer(receiver, pool.address, amountTokenB);
        const expectedLiquidity = Blockchain.expandToDecimal(2, Math.min(token0.decimals, token1.decimals));
        const mint = await pool.mintPool(receiver);
        if (!mint.response)
    ...
    await vm.it(`should get input price`, async () => {
        for (const swapTestCase of swapTestCases) {
            ...
            const end = Date.now();
            Blockchain.restore();
            vm.success(`✔️ Swap test case (${swapAmount}, ${token0Amount}, ${token1Amount}) passed in ${end - start}ms (${swap.usedGas} gas used)`);
        }
    });
    await vm.it('should swap token0', async () => {
        ...
        Assert.expect(balanceWalletToken1).toEqual(totalSupplyToken1 - token1Amount + expectedOutputAmount);
    });
    await vm.it('should swap token1', async () => {
        ...
        const swap = await pool.swap(expectedOutputAmount, 0n, receiver, new Uint8Array());
        const transferEvent = swap.events.shift();
        const syncEvent = swap.events.shift();
        const swapEvent = swap.events.shift();
        if (!transferEvent || !syncEvent || !swapEvent) {
        ...
        const balanceWalletToken0 = await token0.balanceOf(receiver);
        const balanceWalletToken1 = await token1.balanceOf(receiver);
        Assert.expect(balanceWalletToken0).toEqual(totalSupplyToken0 - token0Amount + expectedOutputAmount);
    ...
    await vm.it(`should burn liquidity`, async () => {
        ...
        const burn = await pool.burnLiquidity(receiver);
        if (!burn.response) {
            throw new Error('Response not found');
        }
    ...
    await vm.it(`should have valid price{0,1}CumulativeLast`, async () => {
        ...
        await pool.sync();
        const newPrice = Blockchain.encodePrice(Blockchain.expandTo18Decimals(6), Blockchain.expandTo18Decimals(2));
        ...
        Assert.expect(cumulativePrice0LastAfterSync).toEqual(initialPrice[0] * 10n + newPrice[0] * 10n);
        Assert.expect(cumulativePrice1LastAfterSync).toEqual(initialPrice[1] * 10n + newPrice[1] * 10n);
