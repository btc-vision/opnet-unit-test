...
const dttAddress = Blockchain.generateRandomSegwitAddress();
...
let wbtc;
let router;
async function mintTokens(amountA = 11000000, amountB = 11000000) {
...
function getReserves(tokenA, tokenB, reserve0, reserve1) {
...
await opnet('Motoswap Router', async (vm) => {
    ...
    await vm.it(`should getAmountsOut`, async () => {
        ...
        const path = [WBTC_ADDRESS, dttAddress];
...
await opnet(`Motoswap Router: fee-on-transfer tokens`, async (vm) => {
    vm.beforeEach(async () => {
        Blockchain.clearContracts();
        Blockchain.dispose();
        /** Init factory */
    ...
    await vm.it(`should swapExactTokensForTokensSupportingFeeOnTransferTokens (DTT -> WBTC)`, async () => {
        await addLiquidity(100000n, 100000n);
        await DTT.approve(receiver, router.address, MaxUint256);
        const swapTime = Date.now();
        const response = await router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, 0n, [dttAddress, WBTC_ADDRESS], receiver, 2n);
        vm.log(`swapExactTokensForTokensSupportingFeeOnTransferTokens took ${Date.now() - swapTime}ms (gas ${response.usedGas})`);
    });
    await vm.it(`should swapExactTokensForTokensSupportingFeeOnTransferTokens (WBTC -> DTT)`, async () => {
        await addLiquidity(11000000n, 11000000n);
        await wbtc.approve(receiver, router.address, MaxUint256);
        await router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, 0n, [WBTC_ADDRESS, dttAddress], receiver, 2n);
    });
});
await opnet(`Motoswap Router: liquidity`, async (vm) => {
    vm.beforeEach(async () => {
        dispose();
        Blockchain.clearContracts();
        Blockchain.dispose();
    ...
    await vm.it('should add liquidity: create pool if the pool does not exist and add liquidity.', async () => {
        const token0Amount = Blockchain.expandTo18Decimals(1);
        const token1Amount = Blockchain.expandTo18Decimals(4);
        const expectedLiquidity = Blockchain.expandTo18Decimals(2);
        ...
        const pair = MotoswapPool.createFromRuntime(Blockchain.getContract(poolCreatedEvent.to), WBTC_ADDRESS, dttAddress);
        await pair.init();
        // Decode sync event
